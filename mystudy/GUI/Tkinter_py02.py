        ...
        if src:
            try:
                try:
                    ...
                    singn = tng_signdata_rsa_List(private_key=rsa_private_key, sign_data=src)
                    self.result_data_Text.delete(1.0, END)
                    self.result_data_Text.insert(1.0, singn)
                    self.write_log_to_Text("INFO:str_trans_to_ts success")
            except:
...
def tng_signdata_rsa_List(private_key=None,sign_data=None,redata_list=None):
    if redata_list == 'txnList':
